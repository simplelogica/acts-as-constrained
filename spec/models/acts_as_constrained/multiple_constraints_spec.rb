...
describe ActsAsConstrained do
  ...
  context "when filtering by multiple constraints" do
    ...
    it "should find the offer when filtered by the right constraints" do
      expect(Offer.constrained_by_model(market)).to match_array offers_with_good_market
      expect(Offer.constrained_by_date(Time.now)).to match_array offers_with_good_date
      expect(Offer.constrained_by_date(Time.now).constrained_by_model(market)).to match_array offers_with_good_market_and_country
      expect(
        Offer.constrained_by_date(Time.now).
          constrained_by_model(market).
          constrained_by_model(country)
